...
class ActiveExecutionRegistry:
    ...
    def relinquish(self, registration_token: uuid.UUID) -> None:
        """Drop pending commands for this registration token."""
        with self._commands.mutex:
            self._commands.queue = deque(
                cmd for cmd in self._commands.queue if cmd.registration_token != registration_token
            )
        self._pending = [
            cmd for cmd in self._pending if cmd.registration_token != registration_token
        ]
